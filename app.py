...
from extraction import runner, VARIABLE_GROUPS
...
PLOT_LAYOUT = dict(
    ...
    hoverlabel=dict(bgcolor=C["card"], bordercolor=C["cyan"], font_color=C["text"]),
...
PRODUCT_RESOLUTIONS = [
    ...
    (
        "MODIS",
        "EVI, LST, ET, Fire, BurnedArea, Soil_Moist, NDVI_MO",
        "500 m–1 km",
        C["orange"],
    ),
...
def _col_stats(args):
    ...
    return {
        ...
        "P25": f"{s.quantile(0.25):.4g}" if len(s) else "—",
        "Median": f"{s.median():.4g}" if len(s) else "—",
        "P75": f"{s.quantile(0.75):.4g}" if len(s) else "—",
...
def fig_map(df, indicator=None, show_roads=True, major_roads_only=True):
    ...
    if not lat or not lon:
        return go.Figure().update_layout(
            **PLOT_LAYOUT,
            title="No lat/lon columns found",
        )
...
def fig_timeseries(df, indicators, date_range=None):
    ...
    fig.update_layout(
        **PLOT_LAYOUT,
        title="Time Series — Daily Spatial Mean",
        xaxis_title="Date",
        yaxis_title="Value",
        hovermode="x unified",
        height=460,
...
def fig_correlation(df, indicators):
    ...
    fig.update_layout(
        **PLOT_LAYOUT,
        ...
        yaxis=dict(tickfont=dict(size=10, color=C["text"]), gridcolor=C["border"]),
...
def generate_folium_html(df, indicator=None):
    if not MOD_STATUS["folium"]:
        return (
            "<html><body style='background:#000;color:#526880;font-family:sans-serif;"
    ...
    if indicator and indicator in df.columns:
        plot_df = (
            df.groupby(grp, as_index=False)[indicator]
            .mean()
    ...
    if indicator and indicator in plot_df.columns and _have_branca:
        ...
        if len(vals) > 1:
            ...
            if any(k in ind_l for k in ["temp", "lst", "t2m", "heat", "dew"]):
                ...
                k in ind_l for k in ["ndvi", "evi", "savi", "msavi", "gci", "arvi"]
            ):
    ...
    for _, row in plot_df.iterrows():
        ...
        popup_html = (
            ...
            + (
                f"<b>Cell:</b> {row[gid_col]}<br>"
                if gid_col and gid_col in row.index
                else ""
            )
        ...
        folium.CircleMarker(
            ...
            tooltip=f"{indicator}: {tip_val}"
            if indicator
            else f"({float(row[lat_col]):.4f}, {float(row[lon_col]):.4f})",
        ).add_to(m)
...
def make_report(df):
    ...
    lat_rng = (
        f"{float(df[lat].min()):.4f} → {float(df[lat].max()):.4f}" if lat else "N/A"
    )
    lon_rng = (
        f"{float(df[lon].min()):.4f} → {float(df[lon].max()):.4f}" if lon else "N/A"
    )
    cov_mean = (
        np.mean([(1 - df[c].isna().mean()) * 100 for c in indicators])
        if indicators
        else 0
    )
            ...
            [
                    ...
                    [
                        ...
                        html.Div(
                            [
                                ...
                                html.Div(
                                    "Environmental Exposure Platform",
                                    style={
                                        "fontSize": "7px",
                                        "color": C["muted"],
                ...
                html.Div(
                    [
                        ...
                        html.Span(
                            " Roads DB" if _ROADS_READY else " Roads —",
                            style={
                                "fontSize": "9px",
                                "color": C["green"] if _ROADS_READY else C["muted"],
                            },
    ...
    return card_wrap(
        html.Div(
            [
                ...
                html.Div(
                    [
                        ...
                        dbc.Button(
                            ...
                            style={
                                "background": C["purple"] if nc_avail else C["muted"],
                                "border": "none",
                                "color": C["bg"] if nc_avail else C["surf"],
                                "fontWeight": "700",
                                "borderRadius": "6px",
                                "padding": "9px 20px",
                                "fontSize": "13px",
                ...
                html.Div(
                    "NetCDF requires xarray + netcdf4: pip install xarray netcdf4"
                    if not nc_avail
                    else "",
                    style={
                        "color": C["orange"],
...
def page_pipeline(runner_state=None, grid_csv_name=None, grid_cols=None):
    ...
    csv_badge = html.Div(
        [
            ...
            html.Span(
                grid_csv_name or "No grid file loaded yet",
                style={
                    "color": C["green"] if grid_csv_name else C["muted"],
                    "fontSize": "12px",
                },
            ),
        ],
        ...
        [
                        ...
                        dcc.Upload(
                            ...
                            children=html.Div(
                                [
                                    ...
                                    html.Span(
                                        "  CSV (lat/lon/cell_id) · NC · Parquet",
                                        style={
                                            "fontSize": "11px",
                                            "color": C["border"],
                                        },
                                ...
                                dbc.Col(
                                    [
                                        ...
                                        dbc.RadioItems(
                                            ...
                                            options=[
                                                {
                                                    "label": "Point data (lat/lon columns)",
                                                    "value": "point",
                                                },
                                                {
                                                    "label": "Gridded / NC file (preserve source grid)",
                                                    "value": "grid",
                                                },
                                            ],
                                            value="point",
                    ...
                    [
                        ...
                        dbc.Row(
                            [
                                ...
                                col_field(
                                    "Source/ID col. (optional)",
                                    "cfg-col-src",
                                    ["pid", "src", "source", "participant"],
                                    optional=True,
                                ),
                            ],
                        ),
            ...
            card_wrap(
                html.Div(
                    [
                        html.Div(
                            "3 · Date Range & Workers",
                            className="config-section-header",
                        ),
                        html.Div(
                            "Date range is used only when the grid has NO per-row date column.",
                            style={
                                "color": C["muted"],
                                "fontSize": "11px",
                                "marginBottom": "14px",
    ...
    return html.Div(
        [
            ...
            dbc.Row(
                [
                    dbc.Col(
                        metric_card(len(indicators), "Indicators", C["cyan"]),
                        md=3,
                    ),
                    dbc.Col(
                        metric_card(
                            df[gid].nunique() if gid else "—",
                            "Grid Cells",
                            C["green"],
                        ),
                        md=3,
                    ...
                    dbc.Col(
                        metric_card(f"{len(df):,}", "Total Records", C["orange"]),
                        md=3,
                    ),
    ...
    roads_badge = (
        html.Div(
            [
                html.Span("◉ ", style={"color": C["green"]}),
                html.Span(
                    "OSM Roads DB loaded  ·  2.8 M segments indexed",
                    style={"color": C["green"], "fontSize": "12px"},
                ),
            ],
            style={
        ...
        [
            ...
            dbc.Row(
                [
                    dbc.Col(
                        [
                            ctrl_label("Colour by indicator"),
                            dcc.Dropdown(
                                id="map-indicator",
                                options=opts,
    ...
    folium_status = (
        html.Div(
            [
                html.Span("◉ ", style={"color": C["green"]}),
                html.Span(
                    "folium available — interactive map ready",
                    style={"color": C["green"], "fontSize": "12px"},
                ),
            ],
            style={
        ...
        if MOD_STATUS["folium"]
        else html.Div(
            [
                html.Span("⚠ ", style={"color": C["orange"]}),
                html.Span(
                    "folium not installed — run: pip install folium branca",
                    style={"color": C["orange"], "fontSize": "12px"},
                ),
            ],
            style={"padding": "8px 0", "marginBottom": "10px"},
        ...
        dr_picker = [
            dbc.Col(
                [
                    ...
                    dcc.DatePickerRange(
                        id="ts-date-range",
                        min_date_allowed=str(d0),
                        max_date_allowed=str(d1),
                        start_date=str(d0),
                        end_date=str(d1),
                                ...
                                [
                                    ...
                                    for icon, label, desc, col in [
                                        ...
                                        (
                                            "🌿",
                                            "Vegetation",
                                            "NDVI, EVI, SAVI, DW land cover",
                                            C["green"],
                                        ),
                                        (
                                            "💧",
                                            "Water",
                                            "NDWI, MNDWI, ET, precipitation",
                                            C["cyan"],
                                        ),
                                        (
                                            "🏙",
                                            "Urban",
                                            "NDBI, NDII, Built-up, NTL",
                                            C["purple"],
                                        ),
                                        (
                                            "🌤",
                                            "Atmosphere",
                                            "BLH, MSLP, RH, wind speed",
                                            C["teal"],
                                        ),
                                    ]
                                ],
                            ...
                            card_wrap(
                                html.Div(
                                    [
                                        ...
                                        html.Div(
                                            "Child Health, Environment & Air Quality Index",
                                            style={
                                                "color": C["muted"],
                                                "fontSize": "12px",
                                            },
...
def cb_url_load(n, url):
    ...
    if not MOD_STATUS["requests"]:
        return (
            no_update,
            no_update,
            html.Span("pip install requests", style={"color": C["orange"]}),
        )
    try:
        ...
        n_ind = len(get_indicator_cols(df))
        msg = html.Span(
            f"✓ {fname}  ·  {len(df):,} rows  ·  {n_ind} vars",
            style={"color": C["green"]},
        )
        ...
        return (
            no_update,
            no_update,
            html.Span(f"✗ {str(exc)[:55]}", style={"color": C["red"]}),
        )
...
def cb_grid_csv_upload(contents, filename):
    ...
    return (
        ...
        cols,
        html.Span(
            f"✓ Grid file: {filename}  ({len(cols)} columns)",
            style={"color": C["green"]},
        ),
    )
...
def cb_nav_style(page):
    return ["top-tab top-tab-active" if p == page else "top-tab" for p in ALL_PAGES]
...
def cb_start(
    ...
    if not gee_proj:
        return html.Span(
            "GEE project is required.",
            style={"color": C["red"]},
        ), no_update
    if not grid_csv_b64:
        return html.Span(
            "Upload a grid file first.",
            style={"color": C["red"]},
        ), no_update
    ...
    if not sel_groups:
        return html.Span(
            "Select at least one variable group.",
            style={"color": C["red"]},
        ), no_update
...
def cb_load_results(n):
    ...
    if not path or not os.path.exists(path):
        return (
            no_update,
            html.Span("Result file not found.", style={"color": C["red"]}),
            no_update,
        )
    try:
        df = pd.read_csv(path)
...
def cb_download_nc(n):
    ...
    try:
        ...
        return dict(
            content=base64.b64encode(nc_bytes).decode(),
            filename=fname,
...
def cb_report(n, data):
    ...
    try:
        ...
        return (
            dict(content=html_str, filename=fname, type="text/html"),
