...
def _extract(col_id, band, date, geom, scale_m, fn=None):
    ...
    try:
        return (
            img.select(band)
            .reduceRegion(ee.Reducer.mean(), geom, scale=scale_m, maxPixels=1e13)
            .get(band)
            .getInfo()
        )
    except Exception:
...
def _extract_fallback(col_id, band, date, geom, scale_m, fn=None, max_days=8):
    ...
    for offset in range(max_days + 1):
        for sign in [+1, -1]:
            try_date = (date_dt + pd.DateOffset(days=sign * offset)).strftime(
                "%Y-%m-%d",
            )
...
def _s2_expr(date, geom, expr_str, var_bands, out_name):
    def fn(im):
        ...
        return m.expression(
            expr_str,
            {k: m.select(v) for k, v in var_bands.items()},
        ).rename(out_name)
...
def build_getters(var_groups=None):
    ...
    all_getters = {
        ...
        # ── MODIS ───────────────────────────────────────────────────────────
        "EVI": lambda d, g: _extract_fallback(
            "MODIS/061/MOD13A1",
            "EVI",
            d,
            g,
        ...
        "NDVI_MO": lambda d, g: _extract_monthly(
            "MODIS/061/MOD13A3",
            "NDVI",
            d,
            g,
        ...
        "NDWI_MO": lambda d, g: _extract_fallback(
            "MODIS/061/MOD09A1",
            "NDWI_MO",
            d,
            g,
        ...
        "LST_C": lambda d, g: _extract_fallback(
            "MODIS/061/MOD11A1",
            "LST_C",
            d,
            g,
        ...
        "LSTN_C": lambda d, g: _extract_fallback(
            "MODIS/061/MOD11A1",
            "LSTN_C",
            d,
            g,
        ...
        "ET": lambda d, g: _extract_fallback(
            "MODIS/061/MOD16A2",
            "ET",
            d,
            g,
        ...
        "SPM25": lambda d, g: _extract_fallback(
...
def gap_fill(
    ...
    df = (
        df.sort_values([col_id, date_col])
...
class ExtractionRunner:
    ...
    def get_state(self) -> dict:
        with self._lock:
            ...
            if st["started_at"]:
                elapsed = (
                    datetime.now() - datetime.fromisoformat(st["started_at"])
                ).seconds
    ...
    def _do_extraction(self, cfg: dict):
        ...
        self._log(f"Variables ({len(getters)}): {', '.join(getters.keys())}")
        self._log(
            "Post-extraction: WS, WD10, RH (derived met)"
            + (", EM_m / EH_m / WRND (road metrics)" if cfg.get("roads_shp") else ""),
        )
        ...
        def extract_one(i_row):
            ...
            geom = (
                _pt.buffer(resolution_m / 2).bounds()
            ...
            self._log(
                f"[{prog}/{n_total}] {row[col_id]}  {date_str}  ({self.state['pct']}%)",
            )
            return rec
        ...
        self._log(
            f"Rows: {len(final_df):,}   Cells: {final_df[col_id].nunique():,}"
